...
RUN if [ "$TARGETARCH" = "arm64" ]; then ARCH="arm64"; else ARCH="amd64"; fi && \
    curl -fsSL -O https://github.com/gohugoio/hugo/releases/download/v"$HUGO_VERSION"/{hugo_"$HUGO_VERSION"_checksums.txt,hugo_extended_"$HUGO_VERSION"_linux-"$ARCH".tar.gz} && \
