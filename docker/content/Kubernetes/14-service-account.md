...
- In the above file, the service account `test-service-account` will not be created for the pod `my-pod` and must be created another way.
...
- Display information about the specified service account, including the name of the secret that stores its token.

```sh
kubectl describe secret <secret-name>
```

- Display the token of the service account according to the secret name found in the service account.
