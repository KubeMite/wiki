...
- Create secret using the specified name, key, and value. `--from-literal` can be used multiple times for multiple values.

```sh
kubectl create secret generic <secret-name> --from-file=secret.properties
```

- Create secret using the specified name and data from the specified file.
